...
public class Main
{
    final private static String dataFile = "src/main/resources/map.json";
    private static Scanner scanner;

    private static StationIndex stationIndex;

    private static final Logger searchLogger, errorLogger, exceptionLogger;
    ...
    public static void main(String[] args)
    {
        ...
        for(;;)
        {
            ...
            try {
                from = takeStation("Введите станцию отправления:");
                to = takeStation("Введите станцию назначения:");
            } catch (RuntimeException e) {
                exceptionLogger.log(Level.ERROR, e.getMessage(), e);
                System.exit(0);
            }
    ...
    private static Station takeStation(String message)
    {
        for(;;)
        {
            ...
            if(station != null) {
                searchLogger.info(String.format("Searching station: %s", station.getName()));
                return station;
            }
            String lineL = line.toLowerCase();
            if (lineL.contains("выйти") || lineL.contains("выход") || lineL.contains("exit"))
                throw new RuntimeException("PATIENT DEMANDS TO GET OUTA HERE");
            errorLogger.info(String.format("Wrong station input: %s", line));
